...
function getMenuRow(s_Disabled, s_Event, s_Image, s_Html) {
	...
	s_MenuRow += ">"
	if (s_Image !=""){
		s_MenuRow += "&nbsp;<img border=0 src='Img/"+s_Image+"' width=20 height=20 align=absmiddle "+s_Disabled+">&nbsp;"; //设置菜单图片位置
	}else{
		s_MenuRow += "&nbsp;";
	}
...
function showToolMenu(menu){
	...
	case "edit":		// 编辑菜单
		...
		sMenu += getMenuRow("", "PasteWord()", "pasteword.gif", "从 Word 中粘贴");
		sMenu += sMenuHr;
	...
	case "component":	// 组件菜单
		...
		sMenu += getMenuRow("", "ShowDialog('dialog/flash.htm', 350, 200, true)", "flash.gif", "插入 Flash 动画");
		...
		sMenu += sMenuHr;
		sMenu += getMenuRow("", "ShowDialog('dialog/fieldset.htm', 370, 200, true)", "fieldset.gif", "插入或修改栏目框");
	...
	case "file":		// 文件视图菜单
		...
		//sMenu += getMenuRow("", "ShowDialog('dialog/about.htm','400','220')", "about.gif", "关于 eWebEditor");
		//sMenu += getMenuRow("", "window.open('http://eWebEditor.webasp.net')", "site.gif", "eWebEditor 站点");
		height = 228;
		break;
	case "table":		// 表格菜单
