...
begin
  ...
  begin
   ...
   // thewebview.SetOnConsoleMessage(DoConsoleMessage, self);    //由于要使用一款体积更小的webkit内核，这几行不支持的注释掉 2019.08.28
   // thewebview.SetOnDocumentReady(DocumentReady, self);  //此行原先就注释掉的和使用webkit无关

    //wkeOnDocumentReady2(thewebview,doDucumentReadyCallback,Self); //由于要使用一款体积更小的webkit内核，这几行不支持的注释掉


    thewebview.SetOnWindowClosing(DoWindowClosing, self);
    thewebview.SetOnWindowDestroy(DoWindowDestroy, self);

   // wkeOnLoadUrlBegin(thewebview, DoOnLoadUrlBegin, Self); //由于要使用一款体积更小的webkit内核，这几行不支持的注释掉
    //wkeOnLoadUrlEnd(thewebview, DoOnLoadUrlEnd, self);  //由于要使用一款体积更小的webkit内核，这几行不支持的注释掉
   ...
   // wkeSetNavigationToNewWindowEnable(thewebview, FpopupEnabled);  //由于要使用一款体积更小的webkit内核，这几行不支持的注释掉

    //wkeSetCspCheckEnable(thewebview, False);       //关闭跨域检查  //由于要使用一款体积更小的webkit内核，这几行不支持的注释掉
